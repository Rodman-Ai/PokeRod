...
(function () {
  ...
  // ------------------------------------------------------------------
  // === BIOME TILE PACKS ===============================================
  // ~10 themed multi-char tile codes per biome category. Maps in each
  // biome use these via the array-of-strings tiles[] format (PR #117)
  // to give each region its own visual character.
  // ------------------------------------------------------------------

  function biomeBase(c, x, y, base) { px(c, x, y, TILE, TILE, base); }

  // ----- ROUTE PACK --------------------------------------------------
  regTile('route_grass_a', 'route_grass_a', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); stipple(c,x,y,TILE,TILE,'#3f963f',8,11); });
  regTile('route_grass_b', 'route_grass_b', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#6fc45a'); stipple(c,x,y,TILE,TILE,'#5cae4c',6,17); for(let i=0;i<4;i++) px(c,x+4+i*7,y+8+(i&1)*6,1,2,'#fff8a0'); });
  regTile('route_dirt_a',  'route_dirt_a',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#b88c5a'); stipple(c,x,y,TILE,TILE,'#8d5b32',10,7); });
  regTile('route_dirt_b',  'route_dirt_b',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#d4a878'); stipple(c,x,y,TILE,TILE,'#8d5b32',12,3); px(c,x+10,y+12,3,2,'#5b3826'); px(c,x+20,y+22,3,2,'#5b3826'); });
  regTile('route_signpost','route_signpost',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+15,y+10,2,18,'#7b3a25'); px(c,x+8,y+8,16,8,'#d4a060'); px(c,x+8,y+8,16,1,'#fff0c8'); px(c,x+10,y+11,12,2,'#3a2010'); });
  regTile('route_fence_h', 'route_fence_h', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x,y+12,TILE,2,'#9b6a3a'); px(c,x,y+22,TILE,2,'#9b6a3a'); for(const xx of [4,16,28]) px(c,x+xx,y+8,3,18,'#6b442b'); });
  regTile('route_fence_v', 'route_fence_v', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+14,y,4,TILE,'#6b442b'); px(c,x+8,y+10,16,3,'#9b6a3a'); px(c,x+8,y+22,16,3,'#9b6a3a'); });
  regTile('route_milestone','route_milestone',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+10,y+12,12,16,'#a0a0a8','#606068','#d0d0d8'); px(c,x+13,y+18,6,1,'#181820'); px(c,x+13,y+22,6,1,'#181820'); });
  regTile('route_hedge',   'route_hedge',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#3f8a3a'); stipple(c,x,y,TILE,TILE,'#1c4818',16,3); stipple(c,x,y,TILE,TILE,'#5cae4c',12,7); });
  regTile('route_pond',    'route_pond',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); disc(c,x+16,y+16,11,'#4878d8'); disc(c,x+16,y+16,8,'#5898f0'); disc(c,x+13,y+13,2,'#a8c8f0'); });

  // ----- TOWN PACK ---------------------------------------------------
  regTile('town_cobble_a', 'town_cobble_a', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); for(let yy=0;yy<TILE;yy+=4) for(let xx=(yy/4)%2*2;xx<TILE;xx+=4) px(c,x+xx,y+yy,3,3,'#888078'); });
  regTile('town_cobble_b', 'town_cobble_b', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#c0b8b0'); for(let i=0;i<8;i++) for(let j=0;j<8;j++) px(c,x+i*4+(j&1),y+j*4,3,3,(i+j)%3?'#a8a098':'#888078'); });
  regTile('town_lamp_post','town_lamp_post',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); px(c,x+15,y+12,2,18,'#383038'); px(c,x+12,y+8,8,5,'#f0c020'); disc(c,x+16,y+10,2,'#fff8a0'); });
  regTile('town_planter',  'town_planter',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); shaded(c,x+6,y+14,20,12,'#7b3a25','#4a1c0c','#b85f3a'); for(const xx of [10,16,22]) { px(c,x+xx,y+8,1,6,'#3f963f'); disc(c,x+xx,y+7,2,['#e83838','#f0c020','#a878f0'][[10,16,22].indexOf(xx)]); } });
  regTile('town_well',     'town_well',     TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); shaded(c,x+6,y+10,20,18,'#7a7068','#383028','#a8a098'); disc(c,x+16,y+18,7,'#181820'); disc(c,x+16,y+18,5,'#3060a8'); px(c,x+15,y+4,2,8,'#604030'); });
  regTile('town_clock_pole','town_clock_pole',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); px(c,x+15,y+14,2,16,'#383038'); disc(c,x+16,y+10,7,'#f0e0c0'); disc(c,x+16,y+10,5,'#fff8e8'); px(c,x+16,y+8,1,3,'#181820'); px(c,x+16,y+10,3,1,'#181820'); });
  regTile('town_market_stall','town_market_stall',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); px(c,x+4,y+8,24,4,'#e83838'); for(let i=0;i<6;i++) px(c,x+4+i*4,y+8,2,4,'#f0e0c0'); shaded(c,x+6,y+12,20,14,'#9b6a3a','#5b3826','#d49a5f'); for(const xx of [10,16,22]) px(c,x+xx,y+16,3,3,['#e83838','#f0c020','#3878d8'][[10,16,22].indexOf(xx)]); });
  regTile('town_window_box','town_window_box',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); shaded(c,x+4,y+18,24,8,'#7b3a25','#4a1c0c','#b85f3a'); for(let i=0;i<4;i++){ const cx=x+8+i*5; px(c,cx,y+13,1,5,'#3f963f'); disc(c,cx,y+12,2,['#f098a0','#f0c020','#a878f0','#f8a8c8'][i]); } });
  regTile('town_garden_path','town_garden_path',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+6,y,20,TILE,'#a8a098'); for(let yy=2;yy<TILE;yy+=6) px(c,x+8,y+yy,16,3,'#888078'); });
  regTile('town_signboard','town_signboard',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); shaded(c,x+4,y+6,24,18,'#9b6a3a','#5b3826','#d49a5f'); px(c,x+6,y+10,20,1,'#3a2010'); px(c,x+6,y+14,16,1,'#3a2010'); px(c,x+6,y+18,18,1,'#3a2010'); });

  // ----- CAVE PACK ---------------------------------------------------
  regTile('cave_floor_a',  'cave_floor_a',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5a544c'); stipple(c,x,y,TILE,TILE,'#3a3430',12,5); stipple(c,x,y,TILE,TILE,'#7a7468',8,9); });
  regTile('cave_floor_b',  'cave_floor_b',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#4a4440'); stipple(c,x,y,TILE,TILE,'#2a241e',16,7); for(const i of [0,1,2]) px(c,x+8+i*8,y+12+(i&1)*8,2,2,'#7a7060'); });
  regTile('cave_stalactite','cave_stalactite',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5a544c'); for(let i=0;i<3;i++){ const cx=x+8+i*8, h=8+i*2; for(let j=0;j<h;j++) px(c,cx-((h-j)>>2),y+j,((h-j)>>1)+1,1,'#3a3430'); } });
  regTile('cave_crystal',  'cave_crystal',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#3a3430'); shaded(c,x+10,y+10,12,16,'#a878f0','#583058','#e8c8ff'); px(c,x+13,y+13,2,4,'#fff'); });
  regTile('cave_water_pool','cave_water_pool',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#3a3430'); disc(c,x+16,y+18,11,'#1c2848'); disc(c,x+16,y+18,8,'#3060a8'); px(c,x+13,y+15,4,1,'#5898f0'); });
  regTile('cave_pebble',   'cave_pebble',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5a544c'); for(const [px_,py_,r] of [[8,8,2],[20,12,3],[14,22,2],[24,24,2],[10,18,3]]) disc(c,x+px_,y+py_,r,'#3a3430'); });
  regTile('cave_torch',    'cave_torch',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5a544c'); px(c,x+15,y+12,2,16,'#5b3826'); disc(c,x+16,y+8,4,'#f0c020'); disc(c,x+16,y+6,2,'#fff8e8'); });
  regTile('cave_pillar',   'cave_pillar',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#3a3430'); shaded(c,x+8,y+2,16,28,'#7a7068','#383028','#a8a098'); px(c,x+8,y+8,16,1,'#383028'); px(c,x+8,y+22,16,1,'#383028'); });
  regTile('cave_mushroom', 'cave_mushroom', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5a544c'); px(c,x+15,y+18,2,8,'#f8e8d8'); disc(c,x+16,y+14,5,'#e83838'); disc(c,x+14,y+13,1,'#fff'); disc(c,x+18,y+15,1,'#fff'); });
  regTile('cave_glow_lichen','cave_glow_lichen',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#3a3430'); for(let i=0;i<10;i++){ const ax=x+(i*7+3)%TILE, ay=y+(i*11+5)%TILE; px(c,ax,ay,2,2,'#80f8a0'); }});

  // ----- SNOW PACK ---------------------------------------------------
  regTile('snow_pack_a',   'snow_pack_a',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); stipple(c,x,y,TILE,TILE,'#c8d0e0',8,3); });
  regTile('snow_pack_b',   'snow_pack_b',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#fafcff'); stipple(c,x,y,TILE,TILE,'#c8d0e0',6,5); for(const [px_,py_] of [[8,8],[20,18],[14,24]]) disc(c,x+px_,y+py_,1,'#a8b8d0'); });
  regTile('snow_drift',    'snow_drift',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); for(let i=0;i<TILE;i++){ const h=Math.round(8+4*Math.sin(i/4)); px(c,x+i,y+TILE-h,1,h,'#fafcff'); } });
  regTile('snow_pine_short','snow_pine_short',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); px(c,x+14,y+22,4,8,'#5b3826'); for(let i=0;i<3;i++){ const h=10-i*2,w=12-i*3; const t=y+10-i*4; px(c,x+(TILE-w)/2,t,w,h,'#1c4818'); px(c,x+(TILE-w)/2,t,w,1,'#fafcff'); } });
  regTile('snow_iglooy',   'snow_iglooy',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); disc(c,x+16,y+22,12,'#fafcff'); disc(c,x+16,y+22,10,'#e8eaf0'); for(let i=0;i<3;i++) px(c,x+10+i*5,y+18,3,1,'#c8d0e0'); px(c,x+13,y+22,6,8,'#383040'); });
  regTile('snow_lantern',  'snow_lantern',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); px(c,x+15,y+12,2,18,'#383038'); shaded(c,x+11,y+6,10,8,'#f0c020','#a08018','#fff8a0'); disc(c,x+16,y+10,2,'#fff'); });
  regTile('snow_print',    'snow_print',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); for(const [px_,py_] of [[6,8],[10,14],[6,20],[10,26]]){ px(c,x+px_,y+py_,3,2,'#a8b8d0'); px(c,x+px_-1,y+py_+2,1,1,'#a8b8d0'); px(c,x+px_+3,y+py_+2,1,1,'#a8b8d0'); }});
  regTile('snow_log_pile', 'snow_log_pile', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); for(let i=0;i<3;i++) shaded(c,x+4,y+10+i*6,24,5,'#9b6a3a','#5b3826','#d49a5f'); for(let i=0;i<6;i++) for(let j=0;j<3;j++) disc(c,x+8+i*4,y+12+j*6,1,'#3a2010'); });
  regTile('snow_signpost_iced','snow_signpost_iced',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); px(c,x+15,y+12,2,18,'#5b3826'); px(c,x+8,y+8,16,8,'#9b6a3a'); px(c,x+8,y+8,16,2,'#fafcff'); px(c,x+10,y+12,12,1,'#3a2010'); });
  regTile('snow_frozen_pond','snow_frozen_pond',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); disc(c,x+16,y+16,12,'#a8d8e8'); disc(c,x+16,y+16,10,'#c8e8f0'); for(let i=0;i<6;i++){ const a=i*Math.PI/3; px(c,x+16+Math.round(Math.cos(a)*8),y+16+Math.round(Math.sin(a)*6),3,1,'#fafcff'); }});

  // ----- BEACH PACK --------------------------------------------------
  regTile('beach_sand_a',  'beach_sand_a',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); stipple(c,x,y,TILE,TILE,'#d4b878',8,3); });
  regTile('beach_sand_b',  'beach_sand_b',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f8e0a0'); stipple(c,x,y,TILE,TILE,'#d4b878',5,7); for(let i=0;i<TILE;i+=8) for(let j=0;j<TILE;j+=8) px(c,x+i+(j/8)%2*4,y+j+2,1,1,'#fff8a0'); });
  regTile('beach_seashell','beach_seashell',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); disc(c,x+16,y+16,7,'#f8c8d0'); disc(c,x+16,y+16,5,'#f098a0'); for(let i=0;i<5;i++) px(c,x+13+i,y+12+i,1,8,'#a04060'); });
  regTile('beach_starfish','beach_starfish',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); for(let i=0;i<5;i++){ const a=i*Math.PI*2/5-Math.PI/2, dx=Math.round(Math.cos(a)*7), dy=Math.round(Math.sin(a)*7); for(let r=0;r<7;r++) px(c,x+16+Math.round(dx*r/7),y+16+Math.round(dy*r/7),2,2,'#e8a040'); } disc(c,x+16,y+16,3,'#f0c060'); });
  regTile('beach_palm_short','beach_palm_short',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); px(c,x+15,y+14,2,16,'#7b3a25'); for(let i=0;i<5;i++){ const a=i*Math.PI*2/5,dx=Math.round(Math.cos(a)*8),dy=Math.round(Math.sin(a)*4)-4; for(let r=0;r<8;r++) px(c,x+16+Math.round(dx*r/8),y+10+Math.round(dy*r/8),2,1,'#3f963f'); }});
  regTile('beach_pier_plank','beach_pier_plank',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5898d8'); for(let i=0;i<4;i++) shaded(c,x,y+i*8,TILE,7,'#9b6a3a','#5b3826','#d49a5f'); for(let i=0;i<4;i++) px(c,x,y+i*8+7,TILE,1,'#3a2010'); });
  regTile('beach_buoy',    'beach_buoy',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5898d8'); disc(c,x+16,y+16,8,'#e83838'); disc(c,x+16,y+16,6,'#fff'); disc(c,x+16,y+16,4,'#e83838'); px(c,x+15,y+8,2,4,'#383038'); });
  regTile('beach_umbrella','beach_umbrella',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); px(c,x+15,y+14,2,16,'#383038'); for(let i=0;i<14;i++) px(c,x+2+i*2,y+8-Math.abs(i-7),2,2,(i&1)?'#e83838':'#fff'); });
  regTile('beach_kelp',    'beach_kelp',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#5898d8'); for(let i=0;i<3;i++){ const cx=x+8+i*8; for(let j=0;j<14;j++) px(c,cx+Math.round(Math.sin(j/3)*2),y+18+j-i,2,1,'#1c4818'); }});
  regTile('beach_sand_castle','beach_sand_castle',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); shaded(c,x+8,y+12,16,18,'#d4b878','#a08850','#f8e0a0'); for(const xx of [8,14,20]) shaded(c,x+xx,y+6,4,8,'#d4b878','#a08850','#f8e0a0'); px(c,x+15,y+2,2,5,'#9b6a3a'); });

  // ----- DESERT PACK -------------------------------------------------
  regTile('desert_sand_a', 'desert_sand_a', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); stipple(c,x,y,TILE,TILE,'#c89848',8,3); });
  regTile('desert_sand_b', 'desert_sand_b', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#f0c878'); stipple(c,x,y,TILE,TILE,'#c89848',5,7); for(let i=0;i<5;i++) px(c,x+(i*7+3)%TILE,y+(i*11+5)%TILE,1,1,'#a87830'); });
  regTile('desert_dune',   'desert_dune',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); for(let i=0;i<TILE;i++){ const h=Math.round(10+6*Math.sin(i/5)); px(c,x+i,y+TILE-h,1,h,'#f0d878'); } });
  regTile('desert_cactus_short','desert_cactus_short',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); shaded(c,x+12,y+10,8,18,'#3f963f','#1c4818','#6fc45a'); shaded(c,x+8,y+14,4,8,'#3f963f','#1c4818','#6fc45a'); shaded(c,x+20,y+12,4,10,'#3f963f','#1c4818','#6fc45a'); for(let i=0;i<4;i++) px(c,x+13,y+13+i*4,6,1,'#1c4818'); });
  regTile('desert_skull',  'desert_skull',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); disc(c,x+16,y+16,9,'#fafcff'); disc(c,x+13,y+15,2,'#181820'); disc(c,x+19,y+15,2,'#181820'); px(c,x+13,y+20,7,2,'#181820'); for(let i=0;i<3;i++) px(c,x+14+i*2,y+22,1,3,'#fafcff'); });
  regTile('desert_oasis',  'desert_oasis',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); disc(c,x+16,y+18,11,'#3060a8'); disc(c,x+16,y+18,9,'#5898d8'); for(const [cx,cy] of [[6,10],[26,8]]) { px(c,x+cx,y+cy,1,8,'#7b3a25'); disc(c,x+cx,y+cy-2,3,'#3f963f'); }});
  regTile('desert_ruin_pillar','desert_ruin_pillar',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); shaded(c,x+10,y+4,12,24,'#d4b878','#8d5b32','#f0d890'); for(let i=0;i<4;i++) px(c,x+10,y+8+i*5,12,1,'#8d5b32'); px(c,x+9,y+26,14,3,'#a08850'); });
  regTile('desert_signboard_burnt','desert_signboard_burnt',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); px(c,x+15,y+12,2,18,'#3a2010'); shaded(c,x+6,y+6,20,12,'#5b3826','#2a1208','#7b3a25'); for(let i=0;i<8;i++) px(c,x+8+i*2,y+8+(i&1)*2,1,1,'#181820'); });
  regTile('desert_rock_a', 'desert_rock_a', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); shaded(c,x+8,y+12,16,16,'#a08850','#5b3826','#d4b878'); for(let i=0;i<4;i++) px(c,x+10+i*4,y+18+(i&1)*3,2,1,'#5b3826'); });
  regTile('desert_rock_b', 'desert_rock_b', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#e8c060'); shaded(c,x+12,y+8,12,20,'#7b3a25','#3a2010','#a08850'); shaded(c,x+4,y+18,10,10,'#7b3a25','#3a2010','#a08850'); });

  // ----- MOUNTAIN PACK -----------------------------------------------
  regTile('mtn_rock_a',    'mtn_rock_a',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+6,y+10,20,18,'#605850','#383028','#a8a098'); for(let i=0;i<4;i++) px(c,x+10+i*4,y+18+(i&1)*3,2,2,'#383028'); });
  regTile('mtn_rock_b',    'mtn_rock_b',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); shaded(c,x+10,y+6,12,22,'#605850','#383028','#a8a098'); shaded(c,x+4,y+20,8,8,'#605850','#383028','#a8a098'); });
  regTile('mtn_pine_tall', 'mtn_pine_tall', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#888078'); px(c,x+14,y+24,4,6,'#5b3826'); for(let i=0;i<4;i++){ const h=8-i,w=14-i*3; px(c,x+(TILE-w)/2,y+4+i*5,w,h,'#1c4818'); }});
  regTile('mtn_cliff_face','mtn_cliff_face',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#605850'); for(let yy=0;yy<TILE;yy+=4) px(c,x,y+yy,TILE,1,'#383028'); for(let yy=0;yy<TILE;yy+=8) px(c,x+(yy/8)*4,y+yy+1,4,3,'#888078'); });
  regTile('mtn_eagle_perch','mtn_eagle_perch',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+8,y+18,16,10,'#605850','#383028','#a8a098'); shaded(c,x+12,y+8,8,10,'#383028','#181820','#605850'); px(c,x+13,y+10,2,2,'#f0c020'); });
  regTile('mtn_signpost_painted','mtn_signpost_painted',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); px(c,x+15,y+12,2,18,'#5b3826'); px(c,x+6,y+8,20,8,'#e8a040'); px(c,x+6,y+8,20,1,'#fff8e8'); px(c,x+8,y+11,16,1,'#3a2010'); px(c,x+8,y+13,12,1,'#3a2010'); });
  regTile('mtn_snow_patch','mtn_snow_patch',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#888078'); for(let i=0;i<6;i++){ const cx=x+(i*7+3)%TILE,cy=y+(i*11+5)%TILE; disc(c,cx,cy,2,'#fafcff'); }});
  regTile('mtn_log_bridge','mtn_log_bridge',TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#605850'); for(let i=0;i<5;i++) shaded(c,x,y+4+i*5,TILE,4,'#9b6a3a','#5b3826','#d49a5f'); });
  regTile('mtn_boulder',   'mtn_boulder',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#888078'); disc(c,x+16,y+18,11,'#605850'); disc(c,x+16,y+18,9,'#888078'); disc(c,x+13,y+15,2,'#a8a098'); });
  regTile('mtn_alpine_flower','mtn_alpine_flower',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); for(const [cx,cy,col] of [[8,8,'#a878f0'],[24,12,'#f098a0'],[14,22,'#f0c020'],[22,24,'#5898d8']]) { px(c,x+cx,y+cy+1,1,3,'#3f963f'); disc(c,x+cx,y+cy,2,col); }});

  // ----- INDOOR PACK -------------------------------------------------
  regTile('floor_wood_a',  'floor_wood_a',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); for(let yy=0;yy<TILE;yy+=8) px(c,x,y+yy,TILE,1,'#5b3826'); for(let yy=0;yy<TILE;yy+=8) for(let xx=(yy/8)%2*4;xx<TILE;xx+=8) px(c,x+xx,y+yy+1,1,7,'#7b4828'); });
  regTile('floor_wood_b',  'floor_wood_b',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#b58a4a'); for(let yy=0;yy<TILE;yy+=6) px(c,x,y+yy,TILE,1,'#7b4828'); });
  regTile('wall_panel_a',  'wall_panel_a',  TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a08050'); px(c,x,y,TILE,2,'#7b4828'); px(c,x,y+TILE-2,TILE,2,'#7b4828'); for(const xx of [4,12,20,28]) px(c,x+xx,y+4,1,TILE-8,'#7b4828'); });
  regTile('rug_red',       'rug_red',       TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); shaded(c,x+2,y+4,28,24,'#a01818','#600808','#d83838'); px(c,x+4,y+6,24,1,'#f0c020'); px(c,x+4,y+25,24,1,'#f0c020'); for(let i=0;i<6;i++) px(c,x+5+i*4,y+15,1,1,'#f0c020'); });
  regTile('rug_blue',      'rug_blue',      TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); shaded(c,x+2,y+4,28,24,'#3030a8','#101058','#5858d8'); px(c,x+4,y+6,24,1,'#f0c020'); px(c,x+4,y+25,24,1,'#f0c020'); disc(c,x+16,y+16,3,'#f0c020'); });
  regTile('bookshelf',     'bookshelf',     TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); shaded(c,x+2,y+2,28,28,'#5b3826','#3a2010','#9b6a3a'); for(let r=0;r<3;r++) for(let i=0;i<6;i++) px(c,x+4+i*4,y+5+r*8,3,5,['#a01818','#3030a8','#f0c020','#3f963f','#a878f0','#5b3826'][i]); });
  regTile('picture_frame', 'picture_frame', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#a08050'); shaded(c,x+4,y+4,24,24,'#5b3826','#3a2010','#9b6a3a'); px(c,x+7,y+7,18,18,'#a8c8f0'); px(c,x+13,y+11,6,6,'#f0c890'); px(c,x+11,y+18,10,6,'#a01818'); });
  regTile('vase_potted_b', 'vase_potted_b', TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); shaded(c,x+12,y+16,8,12,'#3878d8','#181858','#5898f0'); for(let i=0;i<3;i++){ px(c,x+13+i*2,y+8,1,8,'#3f963f'); disc(c,x+13+i*2,y+8,2,['#e83838','#f0c020','#a878f0'][i]); }});
  regTile('lamp_table',    'lamp_table',    TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); px(c,x+15,y+18,2,12,'#383038'); shaded(c,x+10,y+8,12,10,'#f0e0a0','#a08818','#fff8e8'); disc(c,x+16,y+13,2,'#fff8a0'); });
  regTile('table_round',   'table_round',   TILE, TILE, (c,x,y)=>{ biomeBase(c,x,y,'#9b6a3a'); disc(c,x+16,y+16,11,'#7b4828'); disc(c,x+16,y+16,9,'#a87850'); disc(c,x+16,y+16,5,'#7b4828'); });

  // ------------------------------------------------------------------
  // === ZONE-SPECIFIC TILES (Casino, Amusement Park, Castle) =========
  // 30 tiles per zone. Multi-char codes only; placed via the array
  // tiles[] format in the zone's map definition.
  // ------------------------------------------------------------------

  // ----- CASINO (csi_*) ---------------------------------------------
  regTile('csi_floor_carpet','csi_floor_carpet',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); stipple(c,x,y,TILE,TILE,'#a01828',12,5); for(let i=0;i<4;i++) px(c,x+8+i*4,y+8+(i&1)*8,2,2,'#f0c020'); });
  regTile('csi_floor_marble','csi_floor_marble',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); for(let yy=0;yy<TILE;yy+=8) for(let xx=(yy/8)%2*4;xx<TILE;xx+=8) px(c,x+xx,y+yy,4,4,'#383040'); for(let i=0;i<3;i++) px(c,x+(i*11+3)%TILE,y+(i*7+5)%TILE,1,1,'#f0c020'); });
  regTile('csi_floor_chip','csi_floor_chip',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); for(let r=0;r<2;r++) for(let i=0;i<3;i++) disc(c,x+8+i*8,y+10+r*12,3,(r+i)%2?'#a01818':'#181820'); });
  regTile('csi_slot_machine','csi_slot_machine',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+4,y+2,24,28,'#a01818','#600808','#d83838'); px(c,x+8,y+8,16,10,'#181020'); for(let i=0;i<3;i++){ px(c,x+9+i*5,y+10,4,6,'#f0c020'); px(c,x+10+i*5,y+11,1,1,'#fff'); } px(c,x+10,y+22,12,4,'#383040'); });
  regTile('csi_slot_lever','csi_slot_lever',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+8,y+8,16,20,'#a01818','#600808','#d83838'); px(c,x+24,y+6,4,2,'#a8a098'); px(c,x+26,y+8,2,8,'#a8a098'); disc(c,x+27,y+16,2,'#e83838'); });
  regTile('csi_roulette_table','csi_roulette_table',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); disc(c,x+16,y+16,12,'#3f963f'); disc(c,x+16,y+16,10,'#1c4818'); disc(c,x+16,y+16,7,'#181820'); disc(c,x+16,y+16,5,'#a01818'); for(let i=0;i<8;i++){ const a=i*Math.PI/4; px(c,x+16+Math.round(Math.cos(a)*8),y+16+Math.round(Math.sin(a)*8),1,1,'#f0c020'); }});
  regTile('csi_dealer_post','csi_dealer_post',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+8,y+12,16,16,'#181820','#000','#383040'); px(c,x+10,y+14,12,1,'#f0c020'); px(c,x+10,y+24,12,1,'#f0c020'); disc(c,x+16,y+8,4,'#f0c890'); });
  regTile('csi_velvet_rope','csi_velvet_rope',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); for(const xx of [6,26]) { px(c,x+xx,y+8,3,18,'#f0c020'); disc(c,x+xx+1,y+8,2,'#fff8a0'); } px(c,x+8,y+14,18,3,'#a01828'); });
  regTile('csi_neon_marquee','csi_neon_marquee',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); px(c,x+4,y+8,24,16,'#181020'); px(c,x+5,y+9,22,14,'#a8c8f0'); px(c,x+8,y+12,4,6,'#e848a8'); px(c,x+14,y+12,4,6,'#f0c020'); px(c,x+20,y+12,4,6,'#3878d8'); for(let i=0;i<4;i++) px(c,x+8+i*5,y+24,1,2,'#f0c020'); });
  regTile('csi_jukebox','csi_jukebox',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+6,y+2,20,28,'#9b6a3a','#5b3826','#d49a5f'); px(c,x+10,y+6,12,8,'#a8d0e8'); disc(c,x+16,y+10,3,'#e83030'); disc(c,x+16,y+10,1,'#fff'); px(c,x+8,y+16,16,6,'#181018'); for(let i=0;i<8;i++) px(c,x+9+i*2,y+18,1,2,'#f0c020'); });
  regTile('csi_vip_curtain','csi_vip_curtain',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); px(c,x,y+2,TILE,2,'#f0c020'); for(let i=0;i<5;i++) shaded(c,x+i*6,y+4,5,28,'#702848','#380020','#a04068'); });
  regTile('csi_vip_booth','csi_vip_booth',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+4,y+8,24,20,'#702848','#380020','#a04068'); px(c,x+8,y+12,16,4,'#f0c020'); shaded(c,x+10,y+18,12,8,'#a01818','#600808','#d83838'); });
  regTile('csi_prize_counter','csi_prize_counter',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+2,y+8,28,18,'#9b6a3a','#5b3826','#d49a5f'); for(const [xx,col] of [[6,'#f0c020'],[14,'#a01818'],[22,'#3878d8']]) px(c,x+xx,y+12,4,4,col); px(c,x+2,y+8,28,1,'#d49a5f'); });
  regTile('csi_cashier_post','csi_cashier_post',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+4,y+12,24,16,'#383040','#181820','#605870'); px(c,x+8,y+16,16,8,'#a8d0e8'); px(c,x+10,y+18,4,2,'#181820'); px(c,x+18,y+18,4,2,'#181820'); px(c,x+12,y+22,8,1,'#181820'); });
  regTile('csi_jackpot_pillar','csi_jackpot_pillar',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+10,y+4,12,24,'#f0c020','#a08018','#fff8a0'); disc(c,x+16,y+10,5,'#fff'); disc(c,x+16,y+10,3,'#e83838'); for(let i=0;i<4;i++) px(c,x+11+i*3,y+22,2,2,'#fff8a0'); });
  regTile('csi_dice_floor','csi_dice_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+6,y+6,10,10,'#fff','#a8a098','#fff8e8'); px(c,x+10,y+10,2,2,'#181820'); shaded(c,x+18,y+18,10,10,'#fff','#a8a098','#fff8e8'); px(c,x+20,y+20,2,2,'#181820'); px(c,x+24,y+20,2,2,'#181820'); px(c,x+20,y+24,2,2,'#181820'); px(c,x+24,y+24,2,2,'#181820'); });
  regTile('csi_chip_stack_red','csi_chip_stack_red',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); for(let i=0;i<6;i++){ const cy=y+24-i*3; px(c,x+8,cy,16,3,'#a01818'); px(c,x+8,cy,16,1,'#e83838'); }});
  regTile('csi_chip_stack_blue','csi_chip_stack_blue',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); for(let i=0;i<6;i++){ const cy=y+24-i*3; px(c,x+8,cy,16,3,'#283878'); px(c,x+8,cy,16,1,'#3878d8'); }});
  regTile('csi_drink_table','csi_drink_table',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); disc(c,x+16,y+22,8,'#383040'); disc(c,x+16,y+22,6,'#605870'); px(c,x+12,y+8,8,12,'#a8d0e8'); px(c,x+12,y+12,8,4,'#f0c020'); disc(c,x+18,y+6,1,'#e83838'); });
  regTile('csi_casino_door','csi_casino_door',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+6,y+2,20,28,'#9b6a3a','#5b3826','#d49a5f'); px(c,x+8,y+4,16,2,'#f0c020'); for(let i=0;i<7;i++) px(c,x+8,y+8+i*3,16,1,'#3a2010'); disc(c,x+22,y+18,1,'#f0c020'); });
  regTile('csi_vault_door','csi_vault_door',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+4,y+4,24,24,'#605870','#383040','#a8a098'); disc(c,x+16,y+16,9,'#383040'); disc(c,x+16,y+16,7,'#605870'); for(let i=0;i<8;i++){ const a=i*Math.PI/4; px(c,x+16+Math.round(Math.cos(a)*9),y+16+Math.round(Math.sin(a)*9),1,1,'#181820'); }});
  regTile('csi_neon_arrow','csi_neon_arrow',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); px(c,x+4,y+12,22,6,'#e848a8'); px(c,x+5,y+13,20,4,'#f898d0'); px(c,x+22,y+8,6,14,'#e848a8'); px(c,x+24,y+11,4,8,'#f898d0'); px(c,x+16,y+6,6,4,'#a02868'); });
  regTile('csi_lounge_seat','csi_lounge_seat',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+4,y+8,24,18,'#702848','#380020','#a04068'); shaded(c,x+6,y+14,20,10,'#a04068','#702848','#c87090'); px(c,x+4,y+12,2,12,'#702848'); px(c,x+26,y+12,2,12,'#702848'); });
  regTile('csi_dance_floor','csi_dance_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); for(let yy=0;yy<TILE;yy+=8) for(let xx=(yy/8)%2*4;xx<TILE;xx+=8) px(c,x+xx,y+yy,4,4,['#e848a8','#f0c020','#3878d8','#3f963f'][((yy/8)+(xx/4))%4]); });
  regTile('csi_bar_top','csi_bar_top',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x,y+10,TILE,16,'#9b6a3a','#5b3826','#d49a5f'); px(c,x,y+10,TILE,2,'#f0c020'); for(let i=0;i<3;i++){ px(c,x+6+i*8,y+14,4,8,'#a8d0e8'); px(c,x+6+i*8,y+15,4,3,'#f0c020'); }});
  regTile('csi_bar_stool','csi_bar_stool',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); disc(c,x+16,y+12,7,'#a01818'); disc(c,x+16,y+12,5,'#d83838'); px(c,x+15,y+18,2,12,'#383038'); px(c,x+10,y+28,12,2,'#383038'); });
  regTile('csi_mirror_wall','csi_mirror_wall',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); shaded(c,x+4,y+4,24,24,'#f0c020','#a08018','#fff8a0'); px(c,x+7,y+7,18,18,'#a8d0e8'); px(c,x+7,y+7,6,6,'#fff'); px(c,x+19,y+19,6,6,'#fff'); });
  regTile('csi_security_cam','csi_security_cam',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); shaded(c,x+10,y+8,12,8,'#383040','#181820','#605870'); disc(c,x+16,y+12,3,'#181820'); disc(c,x+16,y+12,1,'#e83838'); px(c,x+22,y+10,4,4,'#383040'); });
  regTile('csi_exit_sign','csi_exit_sign',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#181820'); px(c,x+6,y+8,20,12,'#3f963f'); px(c,x+6,y+8,20,1,'#80f8a0'); for(let i=0;i<3;i++) px(c,x+10+i*5,y+12,3,4,'#fff'); px(c,x+15,y+22,2,6,'#383038'); });
  regTile('csi_fountain_chip','csi_fountain_chip',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#601020'); disc(c,x+16,y+18,12,'#383040'); disc(c,x+16,y+18,10,'#3878d8'); disc(c,x+16,y+18,7,'#a01818'); for(let i=0;i<8;i++){ const a=i*Math.PI/4; px(c,x+16+Math.round(Math.cos(a)*4),y+18+Math.round(Math.sin(a)*4),1,1,'#f0c020'); } px(c,x+15,y+8,2,8,'#f0c020'); });

  // ----- AMUSEMENT PARK (amu_*) -------------------------------------
  regTile('amu_path_brick','amu_path_brick',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e83030'); for(let yy=0;yy<TILE;yy+=4) for(let xx=(yy/4)%2*2;xx<TILE;xx+=4) px(c,x+xx,y+yy,3,3,'#a01818'); });
  regTile('amu_path_candy','amu_path_candy',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#fff'); for(let i=0;i<TILE;i+=4) px(c,x+i,y,2,TILE,'#e83030'); });
  regTile('amu_ferris_base_l','amu_ferris_base_l',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); for(let i=0;i<TILE;i++) px(c,x+i,y+TILE-1-Math.round((TILE-i)*0.6),1,Math.round((TILE-i)*0.6),'#383040'); for(let i=0;i<3;i++) disc(c,x+TILE-i*4,y+i*4,2,['#e83030','#f0c020','#3878d8'][i]); });
  regTile('amu_ferris_base_r','amu_ferris_base_r',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); for(let i=0;i<TILE;i++) px(c,x+i,y+TILE-1-Math.round(i*0.6),1,Math.round(i*0.6),'#383040'); for(let i=0;i<3;i++) disc(c,x+i*4,y+i*4,2,['#3f963f','#a878f0','#f098a0'][i]); });
  regTile('amu_ferris_pillar','amu_ferris_pillar',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e8eaf0'); shaded(c,x+12,y,8,TILE,'#383040','#181820','#605870'); for(let i=0;i<4;i++) px(c,x+10,y+4+i*7,12,2,'#f0c020'); });
  regTile('amu_carousel_floor','amu_carousel_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e83030'); for(let i=0;i<TILE;i+=8) px(c,x+i,y,4,TILE,'#fff'); for(let i=0;i<TILE;i+=4) px(c,x,y+i,TILE,1,'#a01818'); });
  regTile('amu_carousel_pole','amu_carousel_pole',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e83030'); for(let i=0;i<TILE;i+=4) px(c,x+12,y+i,8,2,'#f0c020'); for(let i=0;i<TILE;i+=4) px(c,x+12,y+i+2,8,2,'#fff'); });
  regTile('amu_carousel_horse_l','amu_carousel_horse_l',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e83030'); shaded(c,x+8,y+12,18,12,'#fff','#a8a098','#fff8e8'); px(c,x+22,y+10,4,6,'#fff'); px(c,x+22,y+11,1,1,'#181820'); px(c,x+10,y+24,2,4,'#5b3826'); px(c,x+22,y+24,2,4,'#5b3826'); });
  regTile('amu_carousel_horse_r','amu_carousel_horse_r',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#e83030'); shaded(c,x+6,y+12,18,12,'#fff','#a8a098','#fff8e8'); px(c,x+6,y+10,4,6,'#fff'); px(c,x+9,y+11,1,1,'#181820'); px(c,x+8,y+24,2,4,'#5b3826'); px(c,x+20,y+24,2,4,'#5b3826'); });
  regTile('amu_coaster_track_h','amu_coaster_track_h',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x,y+10,TILE,4,'#383040'); px(c,x,y+18,TILE,4,'#383040'); for(let i=0;i<8;i++) px(c,x+i*4,y+14,2,4,'#605870'); });
  regTile('amu_coaster_track_v','amu_coaster_track_v',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+10,y,4,TILE,'#383040'); px(c,x+18,y,4,TILE,'#383040'); for(let i=0;i<8;i++) px(c,x+14,y+i*4,4,2,'#605870'); });
  regTile('amu_ticket_booth_l','amu_ticket_booth_l',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+8,y+8,24,20,'#583878','#281028','#7858a8'); px(c,x+12,y+12,16,8,'#a8d0e8'); px(c,x+12,y+18,16,1,'#181028'); px(c,x+14,y+22,12,4,'#f0c020'); });
  regTile('amu_ticket_booth_r','amu_ticket_booth_r',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x,y+8,24,20,'#583878','#281028','#7858a8'); px(c,x+4,y+12,16,8,'#a8d0e8'); px(c,x+4,y+18,16,1,'#181028'); px(c,x+6,y+22,12,4,'#f0c020'); });
  regTile('amu_food_cart','amu_food_cart',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+4,y+16,24,10,'#a06030','#604018','#c89058'); px(c,x+6,y+8,20,8,'#f0c020'); for(let i=0;i<6;i++) px(c,x+6+i*4,y+8,2,8,'#fff'); disc(c,x+9,y+27,2,'#181818'); disc(c,x+23,y+27,2,'#181818'); });
  regTile('amu_balloon_arch','amu_balloon_arch',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); for(let i=0;i<8;i++){ const a=Math.PI*i/7,cx=x+16+Math.round(Math.cos(a-Math.PI/2)*12),cy=y+16-Math.round(Math.sin(a)*12); disc(c,cx,cy,3,['#e83030','#f0c020','#3878d8','#3f963f','#a878f0','#f098a0','#fff','#e83030'][i]); }});
  regTile('amu_queue_rail','amu_queue_rail',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+6,y+12,2,14,'#a8a098'); px(c,x+24,y+12,2,14,'#a8a098'); for(let i=0;i<3;i++) px(c,x+8,y+14+i*4,16,1,'#a01818'); });
  regTile('amu_photo_zone','amu_photo_zone',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+6,y+6,20,22,'#383078','#181028','#5848a8'); px(c,x+10,y+10,12,6,'#a8d0e8'); px(c,x+10,y+16,12,1,'#181028'); px(c,x+12,y+18,8,4,'#f0c020'); px(c,x+14,y+24,4,2,'#181018'); });
  regTile('amu_mascot_statue','amu_mascot_statue',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); disc(c,x+16,y+10,5,'#f098a0'); disc(c,x+13,y+10,1,'#181818'); disc(c,x+19,y+10,1,'#181818'); px(c,x+14,y+12,4,1,'#a04060'); shaded(c,x+11,y+16,10,12,'#e83838','#a01818','#f8a8a8'); });
  regTile('amu_prize_tent','amu_prize_tent',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+4,y+14,24,14,'#e83838'); for(let i=0;i<6;i++) px(c,x+4+i*4,y+14,2,14,'#fff'); px(c,x+14,y+18,4,10,'#101018'); px(c,x+14,y+6,2,8,'#604030'); px(c,x+16,y+4,2,6,'#f0c020'); });
  regTile('amu_fountain_park','amu_fountain_park',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#a8a098'); disc(c,x+16,y+18,12,'#5898d8'); disc(c,x+16,y+18,9,'#a8d0e8'); for(let i=0;i<6;i++) px(c,x+13+i,y+8,1,8,'#3878d8'); });
  regTile('amu_lantern_string','amu_lantern_string',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x,y+8,TILE,1,'#604030'); for(let i=0;i<6;i++){ const cx=x+2+i*5,cy=y+9+(i%2)*2; px(c,cx-1,cy,3,3,['#e83838','#f0c020','#3878d8','#388838','#f098a0','#f8e8a8'][i]); }});
  regTile('amu_popcorn_kiosk','amu_popcorn_kiosk',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+4,y+12,24,14,'#e8e8e8','#a0a0a0','#fff'); px(c,x+6,y+8,20,5,'#e83838'); px(c,x+6,y+8,20,1,'#fff'); for(let i=0;i<5;i++) px(c,x+8+i*4,y+14,2,2,'#f0c020'); });
  regTile('amu_ride_sign','amu_ride_sign',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+15,y+14,2,16,'#5b3826'); shaded(c,x+4,y+4,24,12,'#f0c020','#a08018','#fff8a0'); for(let i=0;i<3;i++) px(c,x+6,y+7+i*3,20,1,'#3a2010'); });
  regTile('amu_sand_pit','amu_sand_pit',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#f0d890'); for(let yy=0;yy<TILE;yy+=2) px(c,x,y+yy,1,1,'#5b3826'); for(let yy=0;yy<TILE;yy+=2) px(c,x+TILE-1,y+yy,1,1,'#5b3826'); for(let i=0;i<8;i++) px(c,x+(i*5+3)%TILE,y+(i*7+5)%TILE,1,1,'#a87830'); });
  regTile('amu_festive_grass','amu_festive_grass',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); for(let i=0;i<8;i++){ const cx=x+(i*7+3)%TILE,cy=y+(i*11+5)%TILE; disc(c,cx,cy,1,['#e83838','#f0c020','#3878d8','#fff'][i%4]); }});
  regTile('amu_candystripe_wall','amu_candystripe_wall',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#fff'); for(let i=-TILE;i<TILE;i+=6){ for(let j=0;j<TILE;j++){ const xx=i+j; if(xx>=0&&xx<TILE) px(c,x+xx,y+j,3,1,'#e83030'); }}});
  regTile('amu_plush_prize_floor','amu_plush_prize_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); for(const [cx,cy,col] of [[8,8,'#f098a0'],[20,12,'#3878d8'],[14,22,'#f0c020']]) { disc(c,x+cx,y+cy,3,col); disc(c,x+cx-1,y+cy-1,1,'#181818'); disc(c,x+cx+1,y+cy-1,1,'#181818'); }});
  regTile('amu_dart_wall','amu_dart_wall',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); shaded(c,x+4,y+4,24,24,'#a06030','#604018','#c89058'); disc(c,x+16,y+16,7,'#fff'); disc(c,x+16,y+16,5,'#e83838'); disc(c,x+16,y+16,3,'#fff'); disc(c,x+16,y+16,1,'#e83838'); });
  regTile('amu_ring_toss','amu_ring_toss',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#5cae4c'); px(c,x+8,y+24,16,4,'#a06030'); for(let i=0;i<3;i++) px(c,x+11+i*5,y+16,2,8,'#604030'); for(let i=0;i<3;i++){ const cx=x+12+i*5,col=['#e83838','#f0c020','#3878d8'][i]; px(c,cx-2,y+14,4,1,col); px(c,cx-2,y+17,4,1,col); }});
  regTile('amu_mascot_floor','amu_mascot_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#fff'); disc(c,x+16,y+18,10,'#f098a0'); disc(c,x+12,y+16,2,'#181818'); disc(c,x+20,y+16,2,'#181818'); px(c,x+13,y+22,6,1,'#a04060'); for(const [cx,cy] of [[6,6],[26,8]]) disc(c,x+cx,y+cy,2,'#a04060'); });

  // ----- CASTLE (cas_*) ----------------------------------------------
  regTile('cas_stone_floor','cas_stone_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); for(let yy=0;yy<TILE;yy+=4) for(let xx=(yy/4)%2*2;xx<TILE;xx+=4) px(c,x+xx,y+yy,3,3,'#605850'); });
  regTile('cas_stone_wall_block','cas_stone_wall_block',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); for(let yy=0;yy<TILE;yy+=8){ const off=(yy/8)%2*8; for(let xx=-off;xx<TILE;xx+=16){ if(xx+15>=0&&xx<TILE) shaded(c,x+Math.max(0,xx),y+yy,Math.min(15,16-Math.max(0,-xx)),7,'#888078','#383028','#a8a098'); }}});
  regTile('cas_throne_dais','cas_throne_dais',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); for(let i=0;i<3;i++) shaded(c,x+4+i*4,y+10+i*5,24-i*8,5,'#a01818','#600808','#d83838'); for(let i=0;i<3;i++) px(c,x+4+i*4,y+10+i*5,24-i*8,1,'#f0c020'); });
  regTile('cas_throne_seat','cas_throne_seat',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+6,y+4,20,22,'#f0c020','#a08018','#fff8a0'); px(c,x+8,y+8,16,12,'#a01818'); px(c,x+14,y+10,4,4,'#f0c020'); });
  regTile('cas_drawbridge_h','cas_drawbridge_h',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#3060a8'); for(let i=0;i<TILE;i+=4) shaded(c,x+i,y,3,TILE,'#9b6a3a','#5b3826','#d49a5f'); for(let i=0;i<TILE;i+=8) px(c,x+i+1,y,1,TILE,'#3a2010'); });
  regTile('cas_moat_water_l','cas_moat_water_l',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#3060a8'); for(let yy=0;yy<TILE;yy+=4) px(c,x,y+yy,TILE,1,'#5898d8'); for(let yy=0;yy<TILE;yy+=8) px(c,x+(yy/8)*4,y+yy+2,3,1,'#a8d0e8'); });
  regTile('cas_moat_water_r','cas_moat_water_r',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#3060a8'); for(let yy=0;yy<TILE;yy+=4) px(c,x,y+yy+1,TILE,1,'#5898d8'); for(let yy=0;yy<TILE;yy+=8) px(c,x+TILE-(yy/8)*4-3,y+yy+2,3,1,'#a8d0e8'); });
  regTile('cas_portcullis_l','cas_portcullis_l',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x,y,16,TILE,'#888078','#383028','#a8a098'); for(let i=4;i<TILE;i+=6) px(c,x+i,y+4,2,TILE-8,'#383028'); for(let yy=4;yy<TILE-4;yy+=4) px(c,x,y+yy,16,1,'#383028'); });
  regTile('cas_portcullis_r','cas_portcullis_r',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+16,y,16,TILE,'#888078','#383028','#a8a098'); for(let i=4;i<TILE;i+=6) px(c,x+18+i,y+4,2,TILE-8,'#383028'); for(let yy=4;yy<TILE-4;yy+=4) px(c,x+16,y+yy,16,1,'#383028'); });
  regTile('cas_battlement','cas_battlement',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x,y+8,TILE,24,'#888078','#383028','#a8a098'); for(let i=0;i<4;i++) shaded(c,x+i*8,y,6,8,'#888078','#383028','#a8a098'); });
  regTile('cas_banner_pole','cas_banner_pole',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+14,y,4,TILE,'#383028','#181820','#605850'); disc(c,x+16,y+2,2,'#f0c020'); });
  regTile('cas_banner_red','cas_banner_red',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+8,y+2,16,24,'#a01818','#600808','#d83838'); px(c,x+14,y+10,4,8,'#f0c020'); for(let i=0;i<4;i++) px(c,x+8+i*4,y+26,2,4,'#a01818'); });
  regTile('cas_banner_blue','cas_banner_blue',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+8,y+2,16,24,'#3030a8','#101058','#5858d8'); disc(c,x+16,y+12,4,'#f0c020'); for(let i=0;i<4;i++) px(c,x+8+i*4,y+26,2,4,'#3030a8'); });
  regTile('cas_candelabra_lit','cas_candelabra_lit',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); px(c,x+14,y+18,4,12,'#f0c020'); px(c,x+12,y+14,8,4,'#f0c020'); for(const cx of [10,16,22]) { px(c,x+cx-1,y+8,2,6,'#fff8e8'); disc(c,x+cx,y+7,1,'#fff8a0'); }});
  regTile('cas_suit_armor','cas_suit_armor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+12,y+2,8,6,'#a8a8b8','#585858','#d0d0d8'); px(c,x+14,y+5,1,1,'#181818'); px(c,x+17,y+5,1,1,'#181818'); shaded(c,x+10,y+8,12,16,'#888898','#383848','#a8a8b8'); px(c,x+6,y+12,4,8,'#888898'); px(c,x+22,y+12,4,8,'#888898'); px(c,x+12,y+24,8,6,'#383838'); });
  regTile('cas_sword_display','cas_sword_display',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+6,y+4,20,24,'#382838','#181018','#583058'); px(c,x+15,y+8,2,12,'#a8a8b8'); px(c,x+13,y+20,6,2,'#604030'); px(c,x+14,y+22,4,2,'#604030'); disc(c,x+16,y+8,1,'#e83030'); });
  regTile('cas_weapon_rack','cas_weapon_rack',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+4,y+4,24,24,'#604030','#3a2010','#a06838'); for(const xx of [8,14,20]){ px(c,x+xx,y+8,2,16,'#a8a8b8'); px(c,x+xx-1,y+8,4,1,'#f0c020'); }});
  regTile('cas_royal_carpet','cas_royal_carpet',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#a01818'); for(let yy=0;yy<TILE;yy+=4) px(c,x,y+yy,TILE,1,'#600808'); for(let i=0;i<3;i++) for(let j=0;j<3;j++) disc(c,x+8+i*8,y+8+j*8,2,'#f0c020'); });
  regTile('cas_royal_wall_panel','cas_royal_wall_panel',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#583058'); shaded(c,x+4,y+4,24,24,'#702848','#380020','#a04068'); for(let i=0;i<4;i++) px(c,x+8+(i&1)*8,y+8+(i>>1)*8,4,4,'#f0c020'); });
  regTile('cas_chapel_pew','cas_chapel_pew',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x,y+10,TILE,8,'#9b6a3a','#5b3826','#d49a5f'); px(c,x,y+18,TILE,3,'#5b3826'); for(let i=0;i<3;i++) px(c,x+8+i*8,y+22,2,8,'#5b3826'); });
  regTile('cas_treasury_vault','cas_treasury_vault',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+4,y+4,24,24,'#383040','#181820','#605870'); disc(c,x+16,y+16,8,'#f0c020'); for(let i=0;i<8;i++){ const a=i*Math.PI/4; px(c,x+16+Math.round(Math.cos(a)*8),y+16+Math.round(Math.sin(a)*8),1,1,'#181820'); }});
  regTile('cas_gold_pile','cas_gold_pile',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); for(let i=0;i<10;i++){ const cx=x+8+(i%4)*4+(i>>2),cy=y+18+(i>>2)*3; disc(c,cx,cy,2,'#f0c020'); }});
  regTile('cas_library_shelf','cas_library_shelf',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+2,y+2,28,28,'#5b3826','#3a2010','#9b6a3a'); for(let r=0;r<3;r++) for(let i=0;i<6;i++) px(c,x+4+i*4,y+5+r*8,3,5,['#a01818','#3030a8','#f0c020','#3f963f','#a878f0','#5b3826'][i]); });
  regTile('cas_training_dummy','cas_training_dummy',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+12,y+18,8,12,'#9b6a3a','#5b3826','#d49a5f'); shaded(c,x+10,y+8,12,12,'#d4b878','#a08850','#f8e0a0'); px(c,x+14,y+12,4,2,'#a01818'); px(c,x+13,y+16,6,1,'#a01818'); });
  regTile('cas_royal_table','cas_royal_table',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+2,y+10,28,8,'#9b6a3a','#5b3826','#d49a5f'); px(c,x+2,y+18,28,2,'#a01818'); px(c,x+6,y+20,2,8,'#5b3826'); px(c,x+24,y+20,2,8,'#5b3826'); disc(c,x+12,y+13,1,'#f0c020'); disc(c,x+20,y+13,1,'#f0c020'); });
  regTile('cas_fireplace','cas_fireplace',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+4,y+4,24,24,'#383028','#181820','#605850'); px(c,x+8,y+14,16,14,'#181018'); disc(c,x+16,y+22,4,'#e83030'); disc(c,x+16,y+20,3,'#f0c020'); disc(c,x+16,y+18,1,'#fff8a0'); });
  regTile('cas_kings_crown_floor','cas_kings_crown_floor',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); px(c,x+8,y+16,16,8,'#f0c020'); px(c,x+8,y+12,2,4,'#f0c020'); px(c,x+14,y+8,4,8,'#f0c020'); px(c,x+22,y+12,2,4,'#f0c020'); disc(c,x+9,y+12,1,'#a01818'); disc(c,x+16,y+8,1,'#3878d8'); disc(c,x+23,y+12,1,'#a01818'); });
  regTile('cas_queen_painting','cas_queen_painting',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); shaded(c,x+4,y+4,24,24,'#604018','#3a2010','#a06838'); px(c,x+7,y+7,18,18,'#a8c8f0'); px(c,x+13,y+10,6,6,'#f098a0'); px(c,x+11,y+16,10,8,'#a878f0'); disc(c,x+16,y+11,1,'#181818'); });
  regTile('cas_scroll_pile','cas_scroll_pile',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#888078'); for(let i=0;i<3;i++){ const cy=y+18-i*4; px(c,x+4,cy,24,5,'#f0e8d0'); px(c,x+4,cy,24,1,'#a08850'); px(c,x+4,cy+4,24,1,'#a08850'); for(let j=0;j<3;j++) px(c,x+8+j*6,cy+2,4,1,'#181018'); }});
  regTile('cas_gargoyle_pillar','cas_gargoyle_pillar',TILE, TILE,(c,x,y)=>{ biomeBase(c,x,y,'#605850'); shaded(c,x+10,y+8,12,22,'#383028','#181820','#605850'); shaded(c,x+8,y+2,16,8,'#605850','#383028','#888078'); px(c,x+11,y+5,2,2,'#181820'); px(c,x+19,y+5,2,2,'#181820'); for(let i=0;i<5;i++) px(c,x+10,y+12+i*4,12,1,'#181820'); });
  ...
  regDecor('produce_stall', TILE, TILE, (c, x, y) => {
    px(c, x + 4, y + 18, 24, 14, '#785030');
    px(c, x + 4, y + 18, 24, 1, '#a07040');
    px(c, x + 4, y + 31, 24, 1, '#382008');
    px(c, x + 6, y + 20, 20, 4, '#e8c890');
    px(c, x + 8, y + 21, 3, 2, '#d83020');
    px(c, x + 13, y + 21, 3, 2, '#f0a020');
    px(c, x + 18, y + 21, 3, 2, '#388838');
    px(c, x + 23, y + 21, 2, 2, '#a02020');
    px(c, x + 3, y + 4, 2, 14, '#604028');
    px(c, x + 27, y + 4, 2, 14, '#604028');
    px(c, x + 2, y + 4, 28, 6, '#388838');
    for (let i = 0; i < 7; i++) {
      const sx = x + 2 + i * 4;
      px(c, sx, y + 4, 2, 6, '#f0d050');
    }
    px(c, x + 2, y + 10, 28, 1, '#202010');
  });

  regDecor('craft_stall', TILE, TILE, (c, x, y) => {
    px(c, x + 4, y + 18, 24, 14, '#604028');
    px(c, x + 4, y + 18, 24, 1, '#805030');
    px(c, x + 4, y + 31, 24, 1, '#281008');
    px(c, x + 6, y + 20, 20, 4, '#a07848');
    px(c, x + 8, y + 21, 2, 2, '#c89858');
    px(c, x + 12, y + 21, 4, 2, '#604028');
    px(c, x + 18, y + 21, 2, 2, '#e8c068');
    px(c, x + 22, y + 21, 3, 2, '#805030');
    px(c, x + 3, y + 4, 2, 14, '#382010');
    px(c, x + 27, y + 4, 2, 14, '#382010');
    px(c, x + 2, y + 4, 28, 6, '#704830');
    for (let i = 0; i < 7; i++) {
      const sx = x + 2 + i * 4;
      px(c, sx, y + 4, 2, 6, '#a07040');
    }
    px(c, x + 2, y + 10, 28, 1, '#201008');
  });

  regDecor('winter_tent', TILE, TILE, (c, x, y) => {
    px(c, x + 4, y + 20, 24, 12, '#788098');
    px(c, x + 4, y + 20, 24, 1, '#a8b0c8');
    px(c, x + 4, y + 31, 24, 1, '#383848');
    for (let i = 0; i < 12; i++) {
      px(c, x + 16 - i, y + 4 + i, (i * 2) + 1, 1, '#e8eef8');
    }
    for (let i = 0; i < 12; i++) {
      px(c, x + 16 - i, y + 4 + i, 1, 1, '#ffffff');
      px(c, x + 16 + i, y + 4 + i, 1, 1, '#a8b0c8');
    }
    px(c, x + 14, y + 20, 4, 8, '#202028');
    px(c, x + 15, y + 22, 2, 1, '#a8b0c8');
    px(c, x + 6, y + 16, 2, 2, '#ffffff');
    px(c, x + 24, y + 16, 2, 2, '#ffffff');
    px(c, x + 15, y + 3, 2, 2, '#f0c020');
  });
